function [suspicious_index lof2] = LOF_maha(A, k,k_index,numneigh, k_dist1,covA)
%
...
%     [k_index, k_dist] = knnsearch(A,A,'k',k+1,'nsmethod','exhaustive','Distance','mahalanobis', 'IncludeTies',true, 'Cov',covA);
...
%     k_dist1 = cell2mat(cellfun(@(x) x(end),k_dist,'UniformOutput',false));
    ...
    for i = 1:n
        lrd_value(i) = lrd(A, i, k_dist1, k_index, numneigh(i),covA);
    end
    %Initialize lof vector
    lof = zeros(n,1);
    lof2=sum(lrd_value(k_index{n})/lrd_value(n))/numneigh(n);
    %Calculate LOF
%     for i = 1:n
...
%=========================================================================
function lrd_value = lrd(A, index_p, k_dist,k_index, numneighbors,covA)
%Calculate the reachability distance for nearest neighbors
Temp = repmat(A(index_p,:), numneighbors, 1) - A(k_index{index_p}, :);
Temp1=[];
for i=1:size(Temp,1)
Temp1=[Temp1; sqrt(Temp(i,:)*pinv(covA)*Temp(i,:)')];
end
Temp2 = sqrt(sum(Temp.^2,2));
reach_dist = max([Temp1 k_dist(k_index{index_p})],[],2);
